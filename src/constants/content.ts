export const APP_SHORT_NAME = 'YFP';
export const APP_NAME = 'Your Financial Plan';
export const APP_DESCRIPTION = 'Budgetting and Financial Planning made easy';
export const PROD_DOMAIN = 'https://yfp.io';

export const GITHUB_NAME = 'mudrila';
export const TWITTER_NAME = 'klimenkomud';

export const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
export const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z\d])[A-Za-z\d\S]{8,}$/;

export const accountTypes = [
  'Income',
  'Debt',
  'SpendingCategory',
  'Credit',
  'Debit',
  'Goal',
] as const;

export const audienceId = '274752ed-c875-4c06-aa9e-984d5b96df11';
export const providerId = 'email-and-password';
export const providerType = 'CredantialsProvider';

export const lightBackground = 'rgba(255, 255, 255, 0.9)';
export const lightBoxShadow = '0 8px 32px rgba(0, 0, 0, 0.08)';
export const lightBorder = 'rgba(255, 255, 255, 0.7)';

export const darkBackground = 'rgba(18, 18, 18, 0.8)';
export const darkBoxShadow = '0 8px 32px rgba(0, 0, 0, 0.24)';
export const darkBorder = 'rgba(255, 255, 255, 0.05)';
